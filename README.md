# -Blinkit-Grocery-Sales-Rating-Analysis-Power-BI-Dashboard

🧠 What This Dashboard Solves
Identifies best-selling categories & weak performers
Highlights outlets contributing highest revenue
Shows items with high demand but low quality
Helps improve inventory management & product strategies
Reveals sales trends over establishment years

🔧 Tools & Technologies Used
Power BI Desktop
DAX Measures & Calculated Columns
Power Query (ETL)
Data Modeling
Excel Dataset (BlinkIT Grocery Data)

🚀 How to Use the Dashboard
Download the .pbix file
Open in Power BI Desktop
Explore slicers → Outlet Type, Location, Size & Item Type
View insights through interactive visuals

📌 Key Features of the Dashboard

🔹 KPI Summary
Total Sales: $1.2M
Avg Sales per Item
Avg Rating (3.9)
5-Star Item Count
Total Number of Items
Low-Rated Item Count

🔹 Outlet Insights
Sales by Outlet Location (Tier 1/2/3)
Performance by Outlet Type & Size
Outlet Establishment Year Trend

🔹 Product Insights
Sales by Item Type
Fat Content distribution & performance
Item Visibility & Weight influence

🔹 Advanced Analytics (DAX-Driven)
Top 30 Selling Items
Bottom 10 Items
High-Rated but Low-Sales Items (missed opportunities)
Low-Rated but High-Sales Items (quality risks)
Ranking system using RANKX
Smart flags for quality and sales segmentation

🧠 What This Dashboard Solves

Identifies best-selling categories & weak performers
Highlights outlets contributing highest revenue
Shows items with high demand but low quality
Helps improve inventory management & product strategies
Reveals sales trends over establishment years

📸 Dashboard Preview

<img width="1227" height="745" alt="image" src="https://github.com/user-attachments/assets/f83908f8-93ed-40d4-b153-0b50f37c33fa" />



📹 Video Preview

https://github.com/user-attachments/assets/2a17d381-fd0c-4815-af84-d5b798ea997a

